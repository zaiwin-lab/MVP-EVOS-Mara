    ...
    <SiteLayout>
      <section className="bg-navy-950 text-white">
        <div className={`${SITE_WRAP} py-10 lg:py-12`}>
          <Link to="/prompt-hub" className="inline-flex items-center gap-1.5 text-xs font-semibold text-navy-200 hover:text-white">
            <Icon name="arrowLeft" className="h-4 w-4" /> Semua Bidang
          </Link>
          <div className="mt-4 flex items-start gap-4">
            ...
            <div>
              <h1 className="font-display text-2xl font-extrabold sm:text-3xl">{area.title}</h1>
              <p className="text-sm font-semibold text-gold-200">{area.titleEn}</p>
              <p className="mt-1 text-sm text-navy-200">{area.blurb}</p>
            </div>
          </div>

          <div className="mt-5 flex flex-wrap items-center gap-3">
            <span className="chip bg-white/10 text-white">{area.missions.length} Prompt Missions</span>
            {participantId && (
              <span className="chip bg-gold-400/20 text-gold-200">{triedInArea}/{area.missions.length} telah dicuba</span>
            )}
          </div>
        </div>
      </section>
      ...
      <section className={`${SITE_WRAP} py-10`}>
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="font-display text-sm font-bold text-navy-900 sm:text-base">{m.title}</div>
                  <div className="truncate text-xs text-navy-400">{m.desc}</div>
                </div>
                <span className="inline-flex shrink-0 items-center gap-1 text-sm font-semibold text-navy-700 group-hover:text-gold-600">
                  <span className="hidden sm:inline">Lihat Prompt</span>
        ...
        <p className="mt-6 rounded-xl bg-sand-100 px-4 py-3 text-xs text-navy-500">
          Petua: Gunakan hasil prompt ini sebagai titik permulaan dan sesuaikan mengikut keperluan koperasi anda. Kecilkan Prompt, Besarkan Impak!
        </p>
      </section>
    </SiteLayout>
  );
