  ...
  async function finish(final: Record<string, number>) {
    ...
    if (participantId) {
      ...
      const payload: AssessmentResult = {
        ...
        answers: final,
        readinessCategory: result.band.label,
        strongestIndicator: strongest?.id ?? "",
        priorityIndicator: result.weakestAreaId,
        recommendations: recArea ? [`Mulakan dengan bidang: ${recArea.title}`] : [],
        summary: result.band.message,
    ...
    <SiteLayout>
      <section className="bg-navy-950 text-white">
        <div className={`${SITE_WRAP} py-8`}>
          <span className="section-eyebrow text-gold-300">Penilaian Kesiapsiagaan AI</span>
          <h1 className="mt-2 font-display text-2xl font-extrabold sm:text-3xl">AI Readiness Snapshot</h1>
          <p className="mt-1 text-sm text-navy-200">Hanya {total} soalan · Takes 2 minit · Panduan, bukan audit formal.</p>
          ...
          <div className="mt-5 flex items-center gap-1.5">
            {READINESS_AREAS.map((a, i) => (
              <div key={a.id} className="flex flex-1 flex-col items-center gap-1">
                ...
                <span className="hidden text-[9px] font-semibold text-navy-300 sm:block">{a.radarLabel}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="flex items-center gap-2 text-navy-400">
        ...
        <span className="text-xs font-bold uppercase tracking-wide">{area.title} · {area.titleEn}</span>
      </div>
      <h2 className="mt-4 font-display text-xl font-extrabold text-navy-900 sm:text-2xl">
        <span className="mr-2 text-gold-500">{index + 1}.</span>{area.question}
      </h2>
      <p className="mt-1 text-sm text-navy-400">{area.questionEn}</p>
        ...
        {area.options.map((o) => {
          ...
          return (
            ...
            >
              ...
              <span>
                <span className="text-sm font-bold text-navy-900">{o.label} <span className="font-normal text-navy-400">/ {o.labelEn}</span></span>
                <span className="mt-0.5 block text-xs text-navy-500">{o.desc}</span>
              </span>
            </button>
          );
        })}
      ...
      <div className="mt-6 flex items-center justify-between">
        {onBack ? (
          <button onClick={onBack} className="btn-outline text-sm"><Icon name="arrowLeft" className="h-4 w-4" /> Sebelumnya</button>
        ) : <span />}
        <span className="text-xs font-semibold text-navy-400">Soalan {index + 1} / {total}</span>
      </div>
    </div>
  );
}
    ...
    <div className="mx-auto max-w-3xl">
      <div className="card p-6 text-center sm:p-8">
        <div className="flex flex-col items-center">
          <ScoreRing score={outcome.score} label={outcome.band.label} size={168} />
          <h2 className="mt-4 font-display text-xl font-extrabold text-navy-900 sm:text-2xl">
            Tahap Kesiapsiagaan: {outcome.band.label}
          </h2>
          <p className="mt-2 max-w-lg text-sm text-navy-500">{outcome.band.message}</p>
        </div>

        <div className="mt-6">
          <div className="text-xs font-bold uppercase tracking-wide text-navy-400">Gambaran Kedudukan Anda</div>
          <RadarChart
            axes={READINESS_AREAS.map((a) => ({ label: a.radarLabel, value: outcome.perArea[a.id] ?? 0, max: 20 }))}
            size={260}
          />
        </div>
      </div>
      ...
      {recArea && (
        <div className={`mt-6 rounded-3xl border p-6 ${ac.card}`}>
          <div className="text-xs font-bold uppercase tracking-wide text-navy-400">Bidang cadangan untuk anda mulakan</div>
          <div className="mt-3 flex items-start gap-4">
            ...
            <div>
              <div className="font-display text-lg font-bold text-navy-900">{recArea.title}</div>
              <p className="text-sm text-navy-500">{recArea.blurb}</p>
            </div>
          </div>
          <Link to={`/prompt-hub/${recArea.id}`} className="btn-gold mt-4 w-full sm:w-auto">
            Mula dengan bidang ini <Icon name="arrowRight" className="h-5 w-5" />
          </Link>
        </div>
      )}

      <div className="mt-6 flex flex-col gap-3 sm:flex-row">
        <Link to="/prompt-hub" className="btn-outline flex-1 justify-center">Lihat Semua 6 Bidang</Link>
        {loggedIn ? (
          <Link to="/my" className="btn-ghost flex-1 justify-center">Ke Ruang Saya</Link>
        ) : (
          <Link to="/check-in" className="btn-ghost flex-1 justify-center">Daftar untuk simpan keputusan</Link>
        )}
      </div>
      {!loggedIn && (
        <p className="mt-3 text-center text-xs text-navy-400">
          Daftar kehadiran untuk menyimpan keputusan ini ke Ruang Saya.
        </p>
      )}
    </div>
  );
